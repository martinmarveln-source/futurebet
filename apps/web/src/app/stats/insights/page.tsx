                  ...
                  {items.map((item: any, idx: number) => {
                    ...
                    const content = (
                      <div className="flex flex-col w-full gap-2">
                        ...
                        {hasNextMatch && (
                          <div className="flex items-center justify-between pl-7 pr-1 w-full text-[10px] sm:text-[11px]">
                            <div className="text-slate-400 truncate pr-2 flex items-center gap-1.5">
                              <span className="px-1 py-0.5 bg-slate-800 rounded font-bold text-[9px] uppercase">Next</span>
                              <span className="truncate max-w-[80px] sm:max-w-[100px]">{item.nextOpponent}</span>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0">
                              {item.opponentStatValue !== null && (
                                <span className="text-slate-400" title={`Opponent ${sec.title} stat (${item.opponentGp} games)`}>
                                  Opp: <span className="text-slate-300 font-bold">{Math.round(item.opponentStatValue)}%</span> <span className="text-[9px] opacity-70">({item.opponentGp}g)</span>
                                </span>
                              )}
                              {item.prediction !== null && (
                                <span className="text-blue-400 font-bold" title="Average Prediction (Team Venue Stat + Opp Venue Stat) / 2">
                                  Pred: {Math.round(item.prediction)}%
                                </span>
                              )}
                              {item.odds !== null && (
                                <span className="text-emerald-400 font-bold ml-1 bg-emerald-500/10 px-1.5 py-0.5 rounded">
                    ...
                    return viewType === 'team' ? (
                      <Link href={`/stats/team/${encodeURIComponent(item.team)}?league=${encodeURIComponent(item.league)}&country=${encodeURIComponent(item.country)}`} key={idx} className="flex flex-col p-2.5 rounded-lg bg-slate-900/50 border border-slate-800 hover:border-slate-700 transition-colors w-full">
                        {content}
                      </Link>
                    ) : (
                      <Link href={`/stats/league/${encodeURIComponent(item.league)}?country=${encodeURIComponent(item.country)}`} key={idx} className="flex flex-col p-2.5 rounded-lg bg-slate-900/50 border border-slate-800 hover:border-slate-700 transition-colors w-full">
                        {content}
                      </Link>
                    )ink";
